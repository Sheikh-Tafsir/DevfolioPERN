...
const SearchUser = () => {
    ...
    const getAlluser = async () =>{
        ...
        const apipath = `${process.env.REACT_APP_API_URI}/user/allusers`;
    ...
    <>
        <div className="flex flex-col justify-center items-center text-white search-user">
            <div className="w-5/6 lg:w-1/3 search-bar-box">
                <form className="flex flex-row w-5/6 lg:h-10 2xl:h-12 mx-auto mt-10 mb-8 search-user-form">
                    ...
                    {searchResults.map((curElem)=>{
                        return(
                            <div className="flex w-5/6 lg:text-sm 2xl:text-lg mx-auto mt-2 pt-2 userCard" key={curElem.id}>
                                <img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEiQmcqzN9KSMx-hxPJfiB3yt59uQhN9R4IqjisfUEitJv9lbQVN14QYLsUfmgiH-AoH2VgTFMdRBaTWa9XXpU9aMV1fveYnRgRsf4peaqt_rCR_qyQ483NgjHHdhfYpOr8axyGWhk3DHw5lAUQkXl6NGMugPS7k6Apw7CUjqRMgwAv01i2_AXyRumuBfw/w680/blank-profile-picture-hd-images-photo.JPG" alt="" className="rounded-full h-11 w-11 lg:h-7 lg:w-7 2xl:h-10 2xl:w-10 ml-2 xl:ml-4" />
                                <p className="w-1/2 ml-3 lg:ml-3 2xl:ml-5 lg:mr-2 2xl:mr-5 pt-2 lg:pt-0.5 2xl:pt-1">{curElem.name}</p>
                                <Button className="bg-orange-600 w-16 h-8 lg:w-14 lg:h-6 2xl:w-20 2xl:h-9 lg:text-sm 2xl:text-lg font-semibold pb-0.5 mt-1 lg:mt-0.5 rounded-sm" onClick={()=>goToUserPortfolio(curElem.name)}>View</Button>
                            </div>
                        )
                    })}
                </div>
